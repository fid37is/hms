...
import { Bell, Sun, Moon, KeyRound, LogOut, ChevronDown } from 'lucide-react';
    ...
    <>
      <header className="h-14 flex items-center justify-between px-6 flex-shrink-0"
        ...
        <div className="flex items-center gap-1">
          ...
          {/* Notifications */}
          <button className="relative w-8 h-8 flex items-center justify-center rounded-md transition-colors"
            style={{ color: 'var(--text-muted)' }}
            onMouseEnter={e => e.currentTarget.style.backgroundColor = 'var(--bg-subtle)'}
            onMouseLeave={e => e.currentTarget.style.backgroundColor = 'transparent'}>
            <Bell size={15} />
          ...
          <div className="relative ml-1" ref={menuRef}>
            ...
            {menuOpen && (
              <div className="absolute right-0 top-full mt-1.5 w-52 rounded-xl shadow-lg z-50 overflow-hidden"
                ...
                {/* Actions */}
                <div className="p-1">
                  <button onClick={() => { setChangePw(true); setMenuOpen(false); }}
