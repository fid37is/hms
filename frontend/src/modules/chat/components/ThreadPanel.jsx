...
export default function ThreadPanel({ conversation, onClose }) {
  const { messages, isTyping, loadMessages, sendMessage, closeConversation, emitTyping, emitStopTyping, socket } = useStaffChat();
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b flex-shrink-0"
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
            {conversation.departments?.icon} {conversation.departments?.name}
          ...
          return (
            <div key={msg.id} className={`flex ${isStaff ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[70%] flex flex-col gap-1 ${isStaff ? 'items-end' : 'items-start'}`}>
                {!isStaff && (
                  <span className="text-xs px-1" style={{ color: 'var(--text-muted)' }}>{msg.sender_name}</span>
                )}
