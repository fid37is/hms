...
const EVENT_TYPE_COLORS = {
  wedding: '#e91e8c', conference: 'var(--brand)', birthday: '#9c27b0',
  corporate: '#1976d2', gala: '#f57c00', exhibition: '#00897b',
...
export default function EventsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn:  () => eventApi.getEvents({ status, event_type: type, search, page, limit: 20 }).then(r => r.data),
    enabled:  tab === 0,
  });
  ...
  const COLUMNS = [
    ...
    { key: 'venue', label: 'Venue',
      render: r => <span className="text-xs" style={{ color: 'var(--text-sub)' }}>{r.event_venues?.name || '—'}</span>,
    },
    { key: 'date', label: 'Date',
      render: r => (
        <div>
    ...
    { key: 'amount', label: 'Total',
      render: r => <span className="text-xs font-medium font-mono" style={{ color: 'var(--text-base)' }}>{formatCurrency(r.total_amount || 0)}</span>,
    },
    { key: 'status', label: 'Status', render: r => <StatusBadge status={r.status} /> },
  ];

  const panelOpen = !!panel;
  const panelTitle = panel?.type === 'form'
    ? (panel.data ? `Edit · ${panel.data.event_no}` : 'New Event')
    : (panel?.data?.title || '');

  return (
    <div className="space-y-4">

      {/* ── Toolbar row: tabs + action button ── */}
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-1 p-1 rounded-xl w-fit"
          style={{ backgroundColor: 'var(--bg-subtle)', border: '1px solid var(--border-soft)' }}>
          {TABS.map((t, i) => (
            <button key={t} onClick={() => { setTab(i); setPanel(null); setVenuePanel(null); }}
        ...
        {/* Action button always top-right, changes per tab, hides when panel open */}
        {tab === 0 && !panel && (
          <button className="btn-primary text-sm flex items-center gap-2"
            onClick={() => openPanel('form', null)}>
            <Plus size={15} /> New Event
          </button>
        )}
        {tab === 2 && !venuePanel && (
          <button className="btn-primary text-sm flex items-center gap-2"
            onClick={() => setVenuePanel({ venue: null })}>
          ...
          <div style={{
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <div className="relative" style={{ maxWidth: 260, flex: 1 }}>
                ...
                <span className="text-xs ml-auto" style={{ color: 'var(--text-muted)' }}>{total} events</span>
              </div>
      ...
      {/* ── Venues tab ── */}
      {tab === 2 && <VenuesPanel panel={venuePanel} setPanel={setVenuePanel} />}
    </div>
  );
}