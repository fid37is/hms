...
export const getAllSuppliers = async (req, res, next) => {
  try {
    const data = await inventoryService.getAllSuppliers(req.query);
...
export const recordMovement = async (req, res, next) => {
  try {
    const data = await inventoryService.recordMovement(req.params.id, req.body, req.user.sub);
...
export const getAllPurchaseOrders = async (req, res, next) => {
  try {
    ...
    const { data, total } = await inventoryService.getAllPurchaseOrders(
      { status, supplier_id }, Number(page), Number(limit)
    );
...
export const getPurchaseOrderById = async (req, res, next) => {
  try {
    const data = await inventoryService.getPurchaseOrderById(req.params.id);
...
export const createPurchaseOrder = async (req, res, next) => {
  try {
    const data = await inventoryService.createPurchaseOrder(req.body, req.user.sub);
...
export const approvePurchaseOrder = async (req, res, next) => {
  try {
    const data = await inventoryService.approvePurchaseOrder(req.params.id, req.user.sub);
...
export const receivePurchaseOrder = async (req, res, next) => {
  try {
    const data = await inventoryService.receivePurchaseOrder(req.params.id, req.user.sub);
...
export const cancelPurchaseOrder = async (req, res, next) => {
  try {
    const data = await inventoryService.cancelPurchaseOrder(req.params.id);
