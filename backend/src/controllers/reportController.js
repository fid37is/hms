...
export const getOccupancyReport = async (req, res, next) => {
  try {
    ...
    if (!date_from || !date_to) return res.status(400).json({ success: false, message: 'date_from and date_to are required.' });
    const data = await reportService.getOccupancyReport(date_from, date_to);
...
export const getRevenueReport = async (req, res, next) => {
  try {
    const { date_from, date_to } = req.query;
    if (!date_from || !date_to) return res.status(400).json({ success: false, message: 'date_from and date_to are required.' });
    const data = await reportService.getRevenueReport(date_from, date_to);
...
export const getNightAudit = async (req, res, next) => {
  try {
    const data = await reportService.getNightAudit(req.query.date);
...
export const getGuestReport = async (req, res, next) => {
  try {
    const { date_from, date_to } = req.query;
    if (!date_from || !date_to) return res.status(400).json({ success: false, message: 'date_from and date_to are required.' });
    const data = await reportService.getGuestReport(date_from, date_to);
